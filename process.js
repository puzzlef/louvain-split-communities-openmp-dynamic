...
const RRESLT = /^\{-(.+?)\/\+(.+?) batchf, (.+?) threads\} -> \{(.+?)ms, (.+?)ms mark, (.+?)ms init, (.+?)ms firstpass, (.+?)ms locmove, (.+?)ms aggr, (.+?)ms split, (.+?) aff, (.+?) iters, (.+?) passes, (.+?) modularity\} (.+)/m;
...
function readLogLine(ln, data, state) {
  ...
  if (ROMPTH.test(ln)) {
    ...
    var [, batch_deletions_fraction, batch_insertions_fraction, num_threads, time, marking_time, initialization_time, first_pass_time, local_moving_phase_time, aggregation_phase_time, splitting_phase_time, affected_vertices, iterations, passes, modularity, technique] = RRESLT.exec(ln);
